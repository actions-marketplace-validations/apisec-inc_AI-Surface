"""Tests for the model gateway detector.

AI infrastructure (k8s / Helm / Terraform / Dockerfile / compose) is covered
separately in ``test_ai_infra.py``; this detector now handles gateways only.
"""
...
from ai_surface.types import CATEGORY_MODEL_GATEWAY
...
def test_litellm_proxy_config() -> None:
    ...
    assert any("multi-model routing layer" in r for r in f.risk_indicators)
...
def test_combined_directory_yields_two_gateway_findings() -> None:
    """Running on the parent ``gateways/`` dir should yield 2 gateway findings:
    LiteLLM and Portkey. Infra surfaces live in the ai_infra detector now."""
    findings = ModelGatewayDetector().detect(str(FIXTURES))
    assert len(findings) == 2
    assert all(f.category == CATEGORY_MODEL_GATEWAY for f in findings)
